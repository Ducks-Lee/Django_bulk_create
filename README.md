...
### create와 bulk_create의 소요 시간 차이가 나는 이유

create()를 1,000번 호출하면 ORM을 통해 1,000번의 개별 INSERT 작업이 수행됩니다. bulk_create()는 여러 데이터를 묶어서 INSERT하기 때문에 DB가 처리해야 하는 쿼리 횟수를 크게 줄일 수 있어 대량 적재에 더 효율적입니다. SQLite는 별도의 DB 서버 없이 파일 기반으로 동작하므로, 여기서의 성능 차이는 네트워크 통신보다는 SQL 실행 및 트랜잭션 처리 횟수 감소의 영향이 큽니다.
